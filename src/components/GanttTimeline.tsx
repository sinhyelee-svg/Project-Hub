...
import { DATE_LIST, PHASE_META, STATUS_META, isRedDay, getCampaignColor } from '../data';
...
export const GanttTimeline: React.FC<GanttTimelineProps> = ({
  ...
  // Set fixed column width for campaign name to fit about 12 Korean characters (150px)
  const campaignWidth = 150;

  // Dynamic sticky left coordinates
  const col0Width = 45; // No
  const col1Left = col0Width; // Campaign
  const col2Left = col0Width + campaignWidth; // Video Name
  const col3Left = col0Width + campaignWidth + 180; // Status
  const col4Left = col0Width + campaignWidth + 180 + 80; // Progress
                ...
                {DATE_LIST.map((date, index) => {
                  ...
                  return (
                    <td
                      ...
                      title={`${video.campaignName ? `[${video.campaignName}] ` : ''}${video.name} - ${date}: ${phase || '지정 없음'}`}
                    >
                      {/* Interactive block overlay */}
                      {phase && (
                        <div 
                          className={`absolute inset-y-1 flex items-center justify-center border overflow-visible z-10
                            ${meta?.border} ${meta?.bg}
                        ...
                        >
                          {!isConnectedLeft && (
                            <span className={`absolute left-1.5 text-[9px] font-extrabold whitespace-nowrap flex items-center gap-1 select-none pointer-events-none z-20 ${meta?.color}`}>
